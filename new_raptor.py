...
class NewRaptor:
    ...
    def run(self):
        ...
        if self.first_start == True:
            ...
            # add an event for when the Species combo box is changed
            self.dlg.cmbSpecies.currentTextChanged.connect(self.evt_cmbSpecies_changed)

        # initialise some of the fields values based on map location and today's date
        ...
        # check to see if the required layers are open in the project
        map_layers = []
        ...
        if result:
            # get reference of the required layers
            ...
            # find the index value of the 'Nest_ID' field
            idxNestID = lyrNests.fields().indexOf("Nest_ID")
            # find the next number of the 'Nest_ID' field by adding 1 to the previous highest value
            ...
            QMessageBox.information(self.dlg, "Message", "NestID = {}\nLat = {}\nLng = {}\nSpecies = {}\n"
                                                         "Buffer = {}\nStatus = {}\nDate = {}"
                                                         "".format(valNestID, valLat, valLng, valSpecies,
                                                                   valBuffer, valStatus, valLast))
            # creates an empty feature based off of lyrNests
            ftrNest = QgsFeature(lyrNests.fields())
            # set the attributes of the feature collected from the plugin dialog box
            ...
            # create data rovider to insert the data into lyrNest
            pr = lyrNests.dataProvider()
            pr.addFeatures([ftrNest])
            # reload the layer so to show the changes
            lyrNests.reload()

            # create the buffer in the raptor buffer layer
            pr = lyrBuffer.dataProvider()
            buffer = geom.buffer(valBuffer, 10)
            # reusing the same fields in the buffer layer so can just set the geometry again
            ftrNest.setGeometry(buffer)
            pr.addFeatures([ftrNest])
            lyrBuffer.reload()

            # show the impacts table dialog once the buffer has been created
            dlgTable = DlgTable()
